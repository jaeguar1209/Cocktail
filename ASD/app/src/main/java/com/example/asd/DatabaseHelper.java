...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public String[] selectData(int _id) {
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = db.rawQuery("SELECT * FROM " + com.example.asd.DataBases.CreateDB._TABLENAME0 + " WHERE _ID = " + _id, null);
        String name="";
        int sugar=0;
        int alcohol=0;
        int body=0;
        ...
        if (cursor.getCount() >= 1) {
            ...
            sugar = cursor.getInt(1);
            alcohol = cursor.getInt(2);
            body = cursor.getInt(3);
            unique_ = cursor.getInt(4);
        ...
        String[] a = {name, Integer.toString(sugar), Integer.toString(alcohol), Integer.toString(body), Integer.toString(unique_),base, Integer.toString(_id)};
        return a;
    }
}